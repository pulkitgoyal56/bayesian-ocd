...
def run_trial(seed):
    ...
    env = TMazeEnv(
        mode=PyBulletClientMode,
        obs="vision",
        seed=seed,
        reward_scales=reward_scales_left,
    )
    task_name = "tmaze"
    ...
    rl_config = {"algorithm": "sac", "gamma": 0.9, "target_entropy": 0}
    train_interval = 5
    batch_size = 60
    seq_len = max_steps
    ...
    while global_step <= max_all_steps:
        ...
        if verbose or episode % 100 == 0:
            logging.info(
                task_name
                + " seed {} -- episode {} (global step {}) : steps {}, total reward {}, reached position {}".format(
        ...
        if episode % 100 == 0:
            if record_internal_states:
                agent.save_episode_data(
                    details_savepath + task_name + "_habitization_{}_episode_{}.mat".format(seed, episode),
        ...
        if args.stage_3_start_step - 200 < global_step <= args.stage_3_start_step:
            if episode_record_before_devaluation >= 1:
                agent.save_episode_data(
                    details_savepath + task_name + "_before_devaluation_{}_episode_{}.mat".format(seed, episode_record_before_devaluation),
                    info=infos[: episode_length + 1],
                )
            ...
            if episode_record_after_devaluation >= 1:
                agent.save_episode_data(
                    details_savepath + task_name + "_after_devaluation_{}_episode_{}.mat".format(seed, episode_record_after_devaluation),
                    info=infos[: episode_length + 1],
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="num steps to record detailed behavior (in the end of training)",
    )
    parser.add_argument(
        "--record_final_z_q",
        type=int,
        default=0,
        help="whether to record the final posterior z after full AIf",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="coefficient of loss function of KLD of z",
    )
    parser.add_argument(
        "--decision_precision_threshold",
        type=float,
        default=0.05,
        help="decision precision threshold",
    )

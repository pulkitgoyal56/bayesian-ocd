...
class THH:
    ...
    def __init__(self, client, baseOrientation=[0, 0, 0, 1], basePosition=[0, 0, 0.5], speed=5.0):
        ...
        self.car = pybullet.loadURDF(fileName=f_name,
                              baseOrientation=baseOrientation,
                              basePosition=basePosition,
                              physicsClientId=client)

        pybullet.changeDynamics(self.car, -1, lateralFriction=0, spinningFriction=0, rollingFriction=0)
    ...
    def apply_action(self, action):
        ...
        pybullet.resetBaseVelocity(objectUniqueId=self.car, linearVelocity=[self.xy_speed[0], self.xy_speed[1], 0])

    def get_position(self):
        # Get the position and orientation of the car in the simulation
        pos, ang = pybullet.getBasePositionAndOrientation(self.car, self.client)
        ang = pybullet.getEulerFromQuaternion(ang)
        pos = pos[:2]
